...
const SideBar = () => {
   ...
   const { getUsers, selectedUser, setSelectedUser, users, 
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 overflow-y-scroll
        ...
        <div className='flex flex-col'>
            {filteredUsers.map((user, index) => (
                 <div onClick={() => {setSelectedUser(user); setUnseenMessages(prev => 
                    ({...prev, [user._id]: 0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded-2xl
