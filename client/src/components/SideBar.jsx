...
const SideBar = ({seletedUser, setSeletedUser }) => {

   const navigate = useNavigate()

  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll
    text-white ${seletedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5'>
        ...
        <div className='flex flex-col'>
            {userDummyData.map((user, index) => (
                 <div onClick={() => {setSeletedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded-2xl
                 cursor-pointer max-sm:text-sm ${seletedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                    ...
                    </div>
                    {
                        ...
                        h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{index}</p>
                    }
                 </div>
            ))}
        </div>
